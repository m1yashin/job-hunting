...
- 原典資料はここに残し、出典つきで確認できる事実は `01_企業別/企業名/企業研究/` に反映する。
- 自分との接点、志望理由、逆質問、面接論点として使う内容は `01_企業別/企業名/` に反映する。
- ガクチカや自己PRの母艦メモはここで育て、面接で使う形になったら `02_共通対策/面接/` の該当メモに反映する。
...
- `deep-research-report/` はAI調査や長文調査の原稿置き場にする。事実として使う内容は `01_企業別/企業名/企業研究/`、選考で使う示唆は `01_企業別/企業名/` に切り出す。
- 事実として使える情報は `01_企業別/企業名/企業研究/` に反映する。
- 選考で使う解釈、自分との接点、志望理由、逆質問は `01_企業別/企業名/` に反映する。
